...
class TestUserModel(BaseTestCase):
    ...
    def test_add_user_duplicate_username(self):
        ...
        duplicate_user = User(username="justatest", email="test@test2.com", password='greaterthaneight')
        db.session.add(duplicate_user)
        self.assertRaises(IntegrityError, db.session.commit)

    def test_add_user_duplicate_email(self):
        add_user("justatest", "test@test.com", 'greaterthaneight')
        duplicate_user = User(username="justanothertest", email="test@test.com", password='greaterthaneight')
        db.session.add(duplicate_user)
        self.assertRaises(IntegrityError, db.session.commit)
